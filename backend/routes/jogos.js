...
router.get('/', async (req, res) => {
  const { modalidade_id, fase, status, grupo_id } = req.query;
  try {
    let query = `
      SELECT j.*,
...
router.put('/:id/placar', authMiddleware, async (req, res) => {
  const { gols_casa, gols_visitante, status } = req.body;
  ...
  try {
    ...
    await pool.query(
      'UPDATE jogos SET gols_casa=?, gols_visitante=?, status=? WHERE id=?',
      [gols_casa ?? jogoAnterior.gols_casa, gols_visitante ?? jogoAnterior.gols_visitante, status || jogoAnterior.status, jogoId]
    );

    // Se o jogo foi encerrado e estava em grupos, atualizar classificação
    if (status === 'encerrado' && jogoAnterior.grupo_id && jogoAnterior.fase === 'grupos') {
      await atualizarClassificacao(jogoAnterior.grupo_id, jogoAnterior.time_casa_id, jogoAnterior.time_visitante_id,
